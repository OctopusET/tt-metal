...
class GatedDeltaNet(LightweightModule):
    ...
    def forward(self, x):
        ...
        # 15. Reshape [1,H,1,D_v] -> [1,1,1,value_dim], pad to [1,1,B_pad,value_dim]
        output_flat = ttnn.reshape(output_gated, [1, 1, 1, self.value_dim])
        ttnn.deallocate(output_gated)
        if B_pad > 1:
            output_flat = ttnn.pad(output_flat, padding=((0, 0), (0, 0), (0, B_pad - 1), (0, 0)), value=0)

        # 16. Output projection
        output = ttnn.linear(output_flat, self.out_proj, compute_kernel_config=self.proj_compute_config)

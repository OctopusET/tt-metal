...
TEST_F(MeshDeviceFixture4x8DispatchAgnostic, ParallelizationBenchmark) {
    ...
    std::cout << "\nWorker grid size: " << worker_grid_size.x << "x" << worker_grid_size.y << " = "
              << (worker_grid_size.x * worker_grid_size.y) << " cores\n";

    // One MeshCoordinateRange per device
    ...
    auto run_benchmark = [&](bool unique_per_program, const std::string& label) {
        log_info(tt::LogTest, "Starting ParallelizationBenchmark - {}", label);

        auto programs = utils::create_benchmark_programs(num_programs, worker_grid_size, unique_per_program);

        // Build single workload: 1 program per device
        auto workload = std::make_shared<MeshWorkload>();
        for (int d = 0; d < num_devices; d++) {
            workload->add_program(devices[d], std::move(*programs[d]));
        }

        // Single enqueue: compile + dispatch all 32 programs
        ...
        double finish_ms = std::chrono::duration<double, std::milli>(t2 - t1).count();
        std::cout << "\n[" << label << "]\n";
        std::cout << "EnqueueMeshWorkload: " << enqueue_ms << " ms\n";
        std::cout << "Finish:              " << finish_ms << " ms\n";
        std::cout << "Total:               " << (enqueue_ms + finish_ms) << " ms\n";
    };

    run_benchmark(true, "UniquePrograms");
    run_benchmark(false, "SharedProgram");
}

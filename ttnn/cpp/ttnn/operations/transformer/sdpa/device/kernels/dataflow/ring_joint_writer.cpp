...
// Row-by-row save of output tiles during SALAD + bulk save of max/sum after K-loop.
template <typename ReaderType, typename TensorAccessorType>
void save_row_by_row_with_stats(
    const PaddedAddrGenerator<ReaderType>& cat_out_generator,
    const TensorAccessorType& stats_writer,
    const TensorTileShape& stats_tile_logical,
    const uint32_t nb,
    ...
    // Bulk drain of max/sum (compute pushes after K-loop via copy_block)

...
void mul_block_bcast_cols_acc(
    ...
    // in1 (cb_exp_max_diff) accumulates cumulatively — read at in1_q_subblock offset.
...
static void sdpa_inner_loop_step(
    ...
    // Rows 0..N-2 are softmax'd in-place; row N-1 has raw matmul output.
    {
        ...
        // When save_cb is set, V matmul + SALAD write to save_cb (cb_out) instead of cur.out.
        // Writer drains save_cb row-by-row to DRAM during SALAD. cur.out stays empty.
        const uint32_t out_cb = save_cb ? save_cb : cur.out;
        ...
        auto salad_correct_row = [&](uint32_t salad_row, uint32_t w_salad, bool last_iter, uint32_t& pushed) {
            ...
            }
            {
                ...
                mul_block_bcast_cols_acc<qktv_subblock_h, vDHt, dst_size>(
                    prev.out, cb_exp_max_diff, out_cb, salad_row, w_salad);
            }
...
void sdpa_standard_v2(
    ...
    for (uint32_t q = 0; q < q_chunks_per_core; q++) {
        ...
        for (uint32_t k_chunk = 0; k_chunk < k_num_chunks; k_chunk++) {
            ...
            // prev.out is already popped row-by-row inside salad_correct_row.
            if (!is_first) {
...
void sdpa_ring_v2(
    ...
    for (uint32_t q = global_q_start; q < global_q_end; q++) {
        ...
        for (uint32_t k_chunk = 0; k_chunk < num_kv_chunks; ++k_chunk) {
            ...
            // On last K-chunk of non-last ring iters (multi-Q), redirect output to cb_out.
            // Writer drains cb_out row-by-row during SALAD, eliminating the post-loop copy_block.
            const uint32_t step_save_cb = (is_last_k && !is_last_ring_iter && q_per_core > 1) ? cb_out : 0;
        ...
        if (q_per_core == 1) {
            ...
            // Only max and sum need the bulk copy_block path.
